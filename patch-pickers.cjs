...
// Extract out DatePickerModal
const datePickerModalStr = `export function DatePickerModal({
  value,
  title,
  onClose,
  onChange,
...
// Extract out PartyPickerModal
const partyPickerModalStr = `export function PartyPickerModal({
  customers,
  suppliers,
  value,
  title,
  ...
  const [partyTab, setPartyTab] = useState<"CUSTOMER" | "SUPPLIER">("CUSTOMER");
  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose} style={{ zIndex: 9999 }}>
      <div className="party-modal" onMouseDown={(e) => e.stopPropagation()} dir="rtl">
        ...
        <h2>{title || (partyTab === "CUSTOMER" ? "العملاء" : "الموردين")}</h2>
        ...
        <div className="party-list">
          ...
          {!activeParties.length && <div className="empty-parties">لا توجد أسماء مسجلة</div>}
        </div>
      </div>
    </div>
  );
...
export function CustomPartyPicker({ value, onChange, customers, suppliers = [], label, className }: { value: string; onChange: (v: string) => void; customers: Party[]; suppliers?: Party[]; label?: string; className?: string }) {
  const [open, setOpen] = useState(false);
...
// Let's refactor StatementFilters.tsx to use these new components instead of repeating logic
const refactoredFiltersStr = `export function StatementFilters({
  customers,
  suppliers,
...
// I will just replace the entire StatementFilters component and append the new ones.
// I need to be careful with the exact match. I will just rewrite the whole file, it's safer.

let fullFileStr = `import React, { useMemo, useState, useEffect } from "react";
...
fs.writeFileSync('src/components/StatementFilters.tsx', fullFileStr);
console.log('Pickers refactored');
